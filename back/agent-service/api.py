...
def build_graph(req: GenerateRequest):
    ...
    try:
        ...
        error_msg = str(e)
        logger.error(f"Agent pipeline error: {error_msg}")

        # 检查是否是JSON解析错误，如果是则降级到Mock数据
        if "Unclosed JSON object" in error_msg or "JSONDecodeError" in error_msg or "JSON object" in error_msg:
            logger.warning("检测到JSON解析错误，降级到Mock数据模式")
            ...
            try:
                ...
                # 读取现有的Mock数据
                mock_file = os.path.join(os.path.dirname(__file__), "..", "mock_data", "sample_graph.json")
                with open(mock_file, 'r', encoding='utf-8') as f:
                    mock_graph = json.load(f)

                # 修改核心概念
                mock_graph["meta"]["core_concept"] = concept
                mock_graph["meta"] = {
                    **mock_graph.get("meta", {}),
                    "source": "mock-service-fallback",
                    "generated_at": "2026-01-21T14:03:29.795861+00:00",
                    "core_concept": concept,
                    "trace_id": trace_id,
                    "error_reason": f"Agent pipeline failed: {error_msg}"
                }
                ...
                return GenerateResponse(
                    ...
                    elapsed_ms=_elapsed_ms(),
                    graph=mock_graph,
                    debug={"error": error_msg, "fallback": "mock"},
                    error=None,
                )
            except Exception as mock_e:
                logger.error(f"Mock数据生成也失败: {mock_e}")
                # 如果Mock数据生成也失败，返回错误
                return GenerateResponse(
                    ok=False,
                    trace_id=trace_id,
                    ...
                    graph=None,
                    debug=None,
                    error={"stage": "internal", "message": f"Agent failed: {error_msg}, Mock failed: {mock_e}"},
                )
        ...
        return GenerateResponse(
            ...
            graph=None,
            debug=None,
            error={"stage": "internal", "message": error_msg},
        )
