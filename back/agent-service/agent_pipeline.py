...
def step0_validate_and_normalize(concept: str, client: LLMClient) -> Dict[str, Any]:
    ...
    """
    # 临时跳过规则过滤，只允许乱码输入通过正常验证
    if "???" in concept and len(concept) > 3:
        return {
            "valid": True,
    ...
    # 暂时关闭json_mode，使用普通文本模式，避免JSON解析问题
    raw = client.chat_completion(messages, temperature=0.0, max_tokens=500, json_mode=False)
    ...
    try:
        ...
        try:
            ...
            logger.error(f"JSON修复也失败: {repair_e}")
            # 如果修复也失败，返回默认值
            obj = {
                "valid": True,
                "normalized_concept": concept.strip(),
                "canonical_en": "",
                "aliases": [],
                "reason": f"JSON解析失败，使用默认值。原始错误: {str(e)}"
            }
