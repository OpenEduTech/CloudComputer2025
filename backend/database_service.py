...
class DBManager:
    def __init__(self):
        ...
        self.CACHE_EXPIRE = {
            'exam': 3600,  # 试卷缓存1小时
            'user_answers': 7200,  # 用户答案缓存2小时
            'mistakes': 1800,  # 错题缓存30分钟
            'stats': 300  # 统计数据缓存5分钟
        }
    ...
    def add_mistake(self, question_data, user_ans, grading_result):
        ...
        if existing:
            update_result = self.mistake_col.update_one(
                {"_id": existing["_id"]},
                {
                    "$inc": {"error_times": 1},
                    "$set": {
            ...
            insert_result = self.mistake_col.insert_one(record)
        
        # 失效错题相关缓存
        self.invalidate_cache('mistakes')
        self.invalidate_cache('stats')

    def get_mistakes(self, user_id="default"):
        """获取错题列表，支持用户隔离"""
        # 尝试从缓存获取
        ...
        # 缓存不存在，从数据库查询
        mistakes = sorted(self.mistake_col.find({}, {"_id": 0}), key=lambda x: x.get("error_times", 0), reverse=True)
