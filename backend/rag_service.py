...
class RAGService:
    ...
    def build_index(self, file_path, file_type="pdf", use_ocr=False):
        ...
        try:
            ...
            # 处理智谱AI API的请求限制（一次最多64个片段）
            from langchain_community.embeddings.zhipuai import ZhipuAIEmbeddings
            if isinstance(self.embeddings, ZhipuAIEmbeddings) and len(splits) > 64:
                print(f"📎 检测到 {len(splits)} 个片段，超出智谱AI API限制，开始分批处理...")
                ...
                for i in range(0, len(texts), batch_size):
                    ...
                    print(f"   处理批次 {i//batch_size + 1}/{(len(texts)+batch_size-1)//batch_size} ({len(batch)}个片段)...")
