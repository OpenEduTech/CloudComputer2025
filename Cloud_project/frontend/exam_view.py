...
def render_exam_page(agent_generator, agent_grader, knowledge_base, db):
    ...
    if 'current_exam' in st.session_state:
        ...
        # 显示倒计时
        countdown_col = st.columns([1, 2, 1])[1]
        with countdown_col:
            hours, remainder = divmod(remaining_time, 3600)
            minutes, seconds = divmod(remainder, 60)
            countdown_text = f"⏰ 考试剩余时间: {int(hours):02d}:{int(minutes):02d}:{int(seconds):02d}"
            
            # 根据剩余时间显示不同颜色
            if remaining_time < 5 * 60:  # 5分钟以下
                countdown_col.error(countdown_text)
            elif remaining_time < 15 * 60:  # 15分钟以下
                countdown_col.warning(countdown_text)
            else:
                countdown_col.info(countdown_text)
        
        # 时间结束自动提交
        if remaining_time <= 0 and not st.session_state.get('exam_submitted'):
            st.error("考试时间已结束，自动提交试卷！")
            # 最后保存一次到Redis
            auto_save_to_redis()
            _handle_submission(st.session_state.current_exam, agent_grader, knowledge_base, db)
            # 提交后清除缓存
        ...
        with st.form("exam_form", clear_on_submit=False):
            ...
            with button_col1:
                if st.form_submit_button("保存答案草稿"):
                    auto_save_to_redis()
...
def _render_results():
    ...
    # 提供重新考试选项
    if st.button("重新考试"):
        for key in ['current_exam', 'user_answers', 'exam_submitted', 'grading_results', 'total_score']:
